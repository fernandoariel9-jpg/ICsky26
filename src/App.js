...
function App() {
  ...
  const [modalImg, setModalImg] = useState(null); // Para mostrar imagen ampliada
  ...
  const SolicitudForm = () => (
    <div className="p-4 max-w-md mx-auto">
      ...
      <form onSubmit={handleSubmit} className="bg-white p-4 rounded-2xl shadow mb-6 flex flex-col space-y-3">
        ...
        {editTask?.imagen && <img src={`data:image/jpeg;base64,${editTask.imagen}`} alt="Foto" className="mt-2 w-24 h-24 rounded-full object-cover cursor-pointer" onClick={()=>setModalImg(editTask.imagen)} />}
        {!editTask?.imagen && form.imagen && <img src={`data:image/jpeg;base64,${form.imagen}`} alt="Foto" className="mt-2 w-24 h-24 rounded-full object-cover cursor-pointer" onClick={()=>setModalImg(form.imagen)} />}
        ...
        {tareasFiltradas.map(t=>(
          <li key={t.id} className={`p-3 rounded-xl shadow-sm flex items-center justify-between ${t.fin?"bg-green-200":"bg-blue-100"}`}>
            <div className="flex items-center space-x-3">
              {t.imagen && <img src={`data:image/jpeg;base64,${t.imagen}`} alt="Foto" className="w-12 h-12 rounded-full object-cover cursor-pointer" onClick={()=>setModalImg(t.imagen)} />}
              <div>
      ...
      {modalImg && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50" onClick={()=>setModalImg(null)}>
          <img src={`data:image/jpeg;base64,${modalImg}`} alt="Ampliada" className="max-h-full max-w-full rounded-xl shadow-lg" />
        </div>
      )}
