...
function Main() {
  ...
  return <App usuarioInicial={usuario} />;
}
...
export default function App({ usuarioInicial }) {
  ...
  const [modo, setModo] = useState("menu"); // "menu", "login", "registro", "supervision"
    ...
    return (
      <div className="p-4 max-w-md mx-auto mt-20 text-center">
        ...
        <h1 className="text-2xl font-bold mb-6">
          Bienvenido al gestor de tareas de INGENIERIA CLINICA
        </h1>
  ...
  if (modo === "login") {
    return (
      <UsuarioLogin
        ...
        switchToRegister={() => setModo("registro")}
      />
    );
  }
  ...
  if (modo === "registro") {
    return (
      <RegistroUsuario
        onRegister={(u) => setUsuario(u)}
        switchToLogin={() => setModo("login")}
      />
    );
  }
...
// ---------- Componente Wrapper de Supervisión ----------
