...
export default function ProtocoloChecklist({ config, setVista, personal }) {
  const {
    codigo,
    tituloCorto,
  ...
  const claveBorrador = (ric01Id) => `${borradorPrefijo || `preventivo:${codigo.toLowerCase()}`}:${ric01Id}`;
  const crearPuntos = () => puntosConfig.map((nombre, i) => ({ orden: i + 1, nombre, estado: "", observaciones: "" }));
  ...
  } = useProtocoloBase({ codigo, personal, defaultDescripcion });
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error(`Error cargando borrador ${codigo}:`, err);
    } finally {
      setBorradorCargado(true);
    }
  }, [datos.ric01_id, borradorCargado, codigo]);

  useEffect(() => {
    if (!borradorCargado || !datos.ric01_id || protocoloId) return;
    localStorage.setItem(
      claveBorrador(datos.ric01_id),
      JSON.stringify({ etapa, indiceActual, puntos, enUso, observaciones })
    );
  }, [borradorCargado, datos.ric01_id, protocoloId, etapa, indiceActual, puntos, enUso, observaciones]);
  ...
  const progreso = etapa === 1 ? 100 : ((indiceActual + 1) / Math.max(puntos.length, 1)) * 100;
  const borrarBorrador = () => datos.ric01_id && localStorage.removeItem(claveBorrador(datos.ric01_id));
  ...
  const seleccionarEstado = (estado) => {
    setPuntos((prev) => prev.map((p, i) => i === indiceActual ? { ...p, estado } : p));
  };
  ...
  const guardar = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || `No se pudo guardar ${codigo}`);

      const id = data[nombreIdRespuesta || `${codigo.toLowerCase()}_id`] || data.id;
      setProtocoloId(id);
      borrarBorrador();
      alert("Mantenimiento preventivo guardado correctamente ✅");
      setMostrarEstadoFinal(true);
    } catch (err) {
      setErrorLocal(err.message || `Error guardando ${codigo}`);
    } finally {
      setGuardando(false);
    }
  };
  ...
  const abrirPDF = () => {
    if (!protocoloId) return alert("Primero debe guardar el mantenimiento.");
    window.open(`${endpoint}/${protocoloId}/pdf`, "_blank");
  };
  ...
  const enviarDrive = async () => {
    ...
    try {
      const res = await fetch(`${endpoint}/${protocoloId}/drive`, {
        method: "POST",
        headers: { "Content-Type": "application/json" }
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "No se pudo enviar a Drive");
      alert(data?.carpeta?.nombre ? `✅ PDF enviado a Google Drive\n\nCarpeta: ${data.carpeta.nombre}` : "✅ PDF enviado a Google Drive");
    } catch (err) {
      alert(err.message || `Error enviando ${codigo} a Drive`);
    } finally {
  ...
  return (
    <>
      <ProtocoloLayout codigo={codigo} tituloCorto={tituloCorto} etapas={etapas} etapa={etapa} progreso={progreso} datos={datos} error={errorLocal || errorBase}>
        {etapa === 0 && (
          <TarjetaEtapa titulo={tituloEtapa} ayuda={ayuda}>
            ...
            <div className="flex gap-2">
              <BotonEstado tipo="CONFORME" activo={puntoActual?.estado === "CONFORME"} onClick={() => seleccionarEstado("CONFORME")} />
              <BotonEstado tipo="NO CONFORME" activo={puntoActual?.estado === "NO CONFORME"} onClick={() => seleccionarEstado("NO CONFORME")} />
              <BotonEstado tipo="NO APLICA" activo={puntoActual?.estado === "NO APLICA"} onClick={() => seleccionarEstado("NO APLICA")} />
            </div>
            ...
            {requiereEnUso && indiceActual === puntos.length - 1 && (
              <div className="mt-5">
        ...
        {etapa === 1 && (
          <ResumenMantenimiento
            ...
            mensajeConforme={mensajeConforme}
            ...
            contadores={{ conformes: resumen.conformes, noConformes: resumen.noConformes.length, noAplica: resumen.noAplica }}
