...
const abrirRIC37 = () => { if (!equipo) return alert("Primero seleccione un equipo."); if (!equipo.mantenimiento_id) return alert("Primero debe iniciar el mantenimiento para poder realizar el RIC37."); localStorage.setItem("tareaActiva", JSON.stringify({ id:equipo.mantenimiento_id, ric01_id:equipo.mantenimiento_id, equipo_id:equipo.id, numero_serie:equipo.numero_serie, descripcion:equipo.descripcion, marca_modelo:equipo.marca_modelo, area:equipo.area, servicio:equipo.servicio, sub_servicio:equipo.sub_servicio, tipo_mantenimiento:equipo.tipo_mantenimiento })); setVista("ric37"); };
...
const guardarMantenimiento = async () => {
  ...
  const mantenimientoContinuar=mantenimientoEnEdicion || (equipo?.estado?.toLowerCase()!=="activo"&&equipo?.mantenimiento_id?{id:equipo.mantenimiento_id,tipo_mantenimiento:equipo.tipo_mantenimiento}:null);
...
<button onClick={verHistorial} className="mt-2 bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-xl w-full">📋 Historial del equipo</button><button onClick={imprimirHistorial} className="mt-3 w-full bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-xl">📄 Imprimir historial</button><button onClick={()=>{setMantenimientoEnEdicion(null);setMostrarForm(true);}} className={`px-4 py-2 rounded-xl w-full mt-3 ${equipo.estado?.toLowerCase()!=="activo"?"bg-yellow-500":"bg-blue-500"} text-white`}>{equipo.estado?.toLowerCase()!=="activo"?"🔧 Continuar mantenimiento":"🛠️ Iniciar mantenimiento"}</button><button onClick={abrirRIC37} disabled={!equipo.mantenimiento_id} className="px-4 py-2 rounded-xl w-full mt-2 bg-blue-600 hover:bg-blue-700 text-white disabled:bg-gray-400 disabled:cursor-not-allowed">⚡ RIC37 - Seguridad eléctrica</button><button onClick={abrirRIC44} className="px-4 py-2 rounded-xl w-full mt-2 bg-orange-600 hover:bg-orange-700 text-white">♻️ RIC44 - Obsolescencia</button>{equipo.estado?.toLowerCase()!=="activo"&&equipo.mantenimiento_id&&<div className="bg-yellow-50 border border-yellow-300 rounded p-2 mt-2 text-sm"><p>🔧 Mantenimiento abierto #{equipo.mantenimiento_id}</p>{equipo.tipo_mantenimiento&&<p>Tipo: {equipo.tipo_mantenimiento}</p>}{equipo.diagnostico&&<p>Diagnóstico: {equipo.diagnostico}</p>}</div>}{Array.isArray(equipo.mantenimientos_abiertos)&&equipo.mantenimientos_abiertos.length>0&&<div className="mt-3"><p className="font-bold text-gray-800 mb-2">🔧 Mantenimientos abiertos ({equipo.mantenimientos_abiertos.length})</p><div className="space-y-2">{equipo.mantenimientos_abiertos.map((m)=><div key={m.id} className="border border-yellow-300 bg-yellow-50 rounded-xl p-3 text-sm"><div className="flex justify-between items-center"><strong>Mantenimiento #{m.id}</strong><span className="text-orange-600 font-semibold">ABIERTO</span></div><p><strong>Tipo:</strong> {m.tipo_mantenimiento||"Sin especificar"}</p>{m.diagnostico&&<p><strong>Diagnóstico:</strong> {m.diagnostico}</p>}{m.fecha&&<p><strong>Inicio:</strong> {formatTimestamp(m.fecha)}</p>}{m.asignado&&<p><strong>Técnico:</strong> {m.asignado}</p>}<div className="flex gap-2 mt-2"><button onClick={()=>continuarMantenimientoAbierto(m)} className="flex-1 bg-yellow-500 hover:bg-yellow-600 text-white px-3 py-2 rounded-xl font-semibold">🔧 Continuar</button><button onClick={()=>abrirFinalizarMantenimiento(m)} className="flex-1 bg-green-600 hover:bg-green-700 text-white px-3 py-2 rounded-xl font-semibold">✅ Finalizar</button></div></div>)}</div></div>}</div>}
{error&&<p className="text-red-500 mt-3">{error}</p>}
{mostrarForm&&<div className="bg-gray-100 p-3 rounded-xl mt-3">{mantenimientoEnEdicion&&<div className="bg-yellow-50 border border-yellow-300 rounded p-2 mb-2"><p className="font-bold">🔧 Continuando mantenimiento #{mantenimientoEnEdicion.id}</p><p>Tipo: <strong>{mantenimientoEnEdicion.tipo_mantenimiento||"Sin especificar"}</strong></p>{mantenimientoEnEdicion.diagnostico&&<p>Diagnóstico: <strong>{mantenimientoEnEdicion.diagnostico}</strong></p>}{mantenimientoEnEdicion.fecha&&<p className="text-sm text-gray-600">Iniciado: <strong>{formatTimestamp(mantenimientoEnEdicion.fecha)}</strong></p>}</div>}{!mantenimientoEnEdicion&&!(equipo?.estado?.toLowerCase()!=="activo"&&equipo?.mantenimiento_id)&&<select value={tipoMantenimiento} onChange={(e)=>handleTipoChange(e.target.value)} className="w-full border p-2 rounded-xl mb-2"><option value="">Seleccionar tipo</option><option value="Correctivo">Correctivo</option><option value="Preventivo">Preventivo</option><option value="Verificación">Verificación</option></select>}{!mantenimientoEnEdicion&&equipo?.estado?.toLowerCase()!=="activo"&&equipo?.mantenimiento_id&&<div className="bg-blue-50 border border-blue-200 rounded p-2 mb-2"><p>🔧 Tipo de mantenimiento:<strong> {equipo.tipo_mantenimiento}</strong></p>{equipo.diagnostico&&<p className="mt-1">📋 Diagnóstico:<strong> {equipo.diagnostico}</strong></p>}{equipo.fecha_inicio&&<p className="mt-1 text-sm text-gray-600">📅 Iniciado:<strong> {formatTimestamp(equipo.fecha_inicio)}</strong></p>}</div>}{!mantenimientoEnEdicion&&!(equipo?.estado?.toLowerCase()!=="activo"&&equipo?.mantenimiento_id)&&tipoMantenimiento==="Correctivo"&&<select value={diagnosticoSeleccionado} onChange={(e)=>setDiagnosticoSeleccionado(e.target.value)} className="w-full border p-2 rounded-xl mb-2"><option value="">Seleccionar diagnóstico</option>{diagnosticos.map((d,i)=><option key={i} value={d.diagnostico}>{d.diagnostico}</option>)}</select>}<textarea value={observaciones} onChange={(e)=>setObservaciones(e.target.value)} placeholder="Ingrese solución / avance realizado..." className="w-full border p-2 rounded-xl mb-2" rows={4}/><button onClick={guardarMantenimiento} className="bg-green-500 text-white px-4 py-2 rounded-xl w-full">💾 Guardar</button></div>}
